...
<div class="min-h-screen" style="background: var(--bg-dark);">
    ...
    <div x-data="dineInModal()">
        <div class="relative min-h-[70vh] flex flex-col items-center justify-center text-center px-6 overflow-hidden"
             style="background: linear-gradient(160deg, #000000 0%, #0a0a0a 40%, #1a1a1a 100%);">
            <div class="absolute inset-0 opacity-5"
                 style="background-image: radial-gradient(circle at 20% 80%, #c9a84c 0%, transparent 50%), radial-gradient(circle at 80% 20%, #d4b96a 0%, transparent 50%);"></div>

            <div class="relative z-10 max-w-lg mx-auto">
                <div class="flex items-center justify-center mb-6">
                    <img src="{{ asset('images/logo.png') }}" alt="Kopi Tiang Alam" class="w-32 h-32 rounded-full object-cover shadow-2xl" style="border: 3px solid #c9a84c;" />
                </div>
                <h1 class="text-4xl font-bold tracking-tight mb-2" style="color: #c9a84c;">Kopi Tiang Alam</h1>
                <p class="text-lg mb-2" style="color: #d4b96a;">Warung kopi modern dengan cita rasa autentik</p>
                <p class="text-sm mb-10" style="color: #8b7a3c;">Pesan langsung dari meja Anda, tanpa menunggu</p>
                ...
                <div class="flex flex-col sm:flex-row gap-4 justify-center">
                    ...
                    <a href="{{ route('menu') }}"
                       ...
                       style="border-color: #c9a84c; color: #c9a84c; background: transparent;">
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
        ...
        <div x-show="show" x-cloak
            ...
            <div class="w-full max-w-sm rounded-2xl overflow-hidden" style="background: #1a1a1a;">

                <!-- Modal Header -->
                <div class="px-5 py-4 flex items-center justify-between"
                     style="background: #0a0a0a;">
                    <div>
                        <p class="font-bold text-base" style="color: #c9a84c;">Pilih Meja</p>
                        <p class="text-xs mt-0.5" style="color: #8b7a3c;">Tap meja tempat Anda duduk</p>
                    </div>
                    <button @click="show = false">
                        <svg class="w-5 h-5" fill="none" stroke="#8b7a3c" viewBox="0 0 24 24">
                ...
                <div x-show="loading" class="grid grid-cols-3 gap-3 p-5">
                    <template x-for="i in 6" :key="i">
                        <div class="h-16 rounded-xl animate-pulse" style="background: #2a2a2a;"></div>
                    </template>
                </div>

                <!-- No tables -->
                <div x-show="!loading && tables.length === 0" x-cloak class="p-8 text-center">
                    <p class="text-sm" style="color: #8b7a3c;">Tidak ada meja tersedia</p>
                </div>
                        ...
                        <template x-for="table in tables" :key="table.id">
                            <button @click="table.status === 'available' && selectTable(table.table_number)"
                                    ...
                                    :style="table.status === 'available'
                                        ? 'border-color: #c9a84c; background: #1a1a1a; cursor: pointer;'
                                        : 'border-color: #2a2a2a; background: #111; cursor: not-allowed; opacity: 0.55;'">
                                <p class="font-bold text-sm" style="color: #c9a84c;"
                                   x-text="table.table_number"></p>
                                <p class="text-xs mt-0.5"
                                   :style="table.status === 'available' ? 'color: #4caf50;' : 'color: #666;'"
                                   x-text="table.status === 'available' ? 'Tersedia' : 'Terisi'"></p>
                            </button>
                        </template>
                    </div>
                    <p class="text-xs text-center mt-4" style="color: #8b7a3c;">
    ...
    <div class="px-4 pt-6 max-w-lg mx-auto" x-data="recentOrders()" x-init="init()">
        ...
        <template x-if="orders.length > 0">
            <div class="mb-6">
                <div class="flex items-center justify-between mb-3">
                    <h2 class="font-bold text-base" style="color: #c9a84c;">Pesanan Anda</h2>
                    <button @click="clearOrders()"
                            class="text-xs px-2 py-1 rounded-lg border"
                            style="border-color: #2a2a2a; color: #8b7a3c;">Hapus Riwayat</button>
                </div>
                <div class="space-y-2">
                    <template x-for="o in orders" :key="o.orderNumber">
                        <a :href="'/order/' + o.orderNumber"
                           ...
                           style="background: #1a1a1a; border-color: #2a2a2a; text-decoration: none;">
                            ...
                            <div class="flex-1 min-w-0">
                                <p class="font-semibold text-sm truncate" style="color: #f5f0e8;"
                                   x-text="o.orderNumber"></p>
                                <p class="text-xs mt-0.5" style="color: #8b7a3c;"
                                   x-text="o.customerName + ' · ' + formatDate(o.createdAt)"></p>
                            </div>
                            <svg class="w-4 h-4 shrink-0" fill="none" stroke="#8b7a3c" viewBox="0 0 24 24">
        ...
        <!-- Manual lookup -->
        <div class="rounded-2xl border p-4 mb-6" style="border-color: #2a2a2a; background: #1a1a1a;">
            <p class="text-sm font-semibold mb-2" style="color: #c9a84c;">Cek Status Pesanan</p>
            <p class="text-xs mb-3" style="color: #8b7a3c;">Masukkan nomor pesanan untuk melihat status</p>
            <div class="flex gap-2">
                <input x-model="searchNumber" type="text" placeholder="KTA-20250516-XXXX"
                       ...
                       style="border-color: #2a2a2a; background: #0a0a0a; color: #f5f0e8;" />
                <button @click="searchOrder()"
                        class="h-10 px-4 rounded-xl text-sm font-semibold shrink-0"
                        style="background: #c9a84c; color: #0a0a0a;">Cek</button>
            </div>
            <p x-show="searchError" x-cloak x-text="searchError"
               class="text-xs mt-2" style="color: #e53935;"></p>
        </div>
    </div>
    ...
    <div class="py-14 px-6 max-w-4xl mx-auto">
        <div class="grid grid-cols-1 sm:grid-cols-3 gap-6">
            ...
            <div class="rounded-2xl p-6 border" style="border-color: #2a2a2a; background: #1a1a1a;">
                ...
                <h3 class="font-semibold text-base mb-1" style="color: #c9a84c;">{{ $f['title'] }}</h3>
                <p class="text-sm leading-relaxed" style="color: #8b7a3c;">{{ $f['desc'] }}</p>
            </div>
            @endforeach
        </div>
    </div>
    ...
    <div class="mx-4 mb-10 rounded-2xl p-6 overflow-hidden relative"
         style="background: linear-gradient(135deg, #0a0a0a 0%, #1a1a1a 100%); border: 1px solid #c9a84c;">
        <div class="relative z-10">
            <span class="inline-block mb-3 text-xs font-semibold px-3 py-1 rounded-full"
                  style="background: #c9a84c; color: #0a0a0a;">Promo Hari Ini</span>
            <h2 class="text-2xl font-bold mb-2" style="color: #c9a84c;">Happy Hours</h2>
            <p class="text-sm mb-4" style="color: #8b7a3c;">Diskon 20% untuk semua minuman kopi pukul 14.00 – 16.00</p>
            <a href="{{ route('menu') }}"
               class="inline-block text-sm font-semibold px-4 py-2 rounded-lg transition-opacity hover:opacity-90"
               style="background: #c9a84c; color: #0a0a0a;">Pesan Sekarang</a>
        </div>
        <div class="absolute -right-6 -top-6 w-32 h-32 rounded-full opacity-10" style="background: #c9a84c;"></div>
        <div class="absolute -right-2 bottom-0 w-20 h-20 rounded-full opacity-10" style="background: #d4b96a;"></div>
    </div>

    <!-- Categories -->
    <div class="px-4 pb-12">
        <h2 class="text-xl font-bold mb-5 px-2" style="color: #c9a84c;">Kategori Menu</h2>
        <div class="grid grid-cols-2 sm:grid-cols-3 gap-3">
            ...
            <a href="{{ route('menu') }}"
               class="rounded-xl p-4 text-left transition-transform active:scale-95 hover:opacity-90 border"
               style="background: #1a1a1a; border-color: #c9a84c; color: #c9a84c;">
                <div class="text-2xl mb-2">{{ $categoryEmojis[$category->name] ?? '🍽️' }}</div>
                <div class="text-sm font-semibold">{{ $category->name }}</div>
            </a>
            @empty
            <p class="col-span-full text-center text-sm" style="color: #8b7a3c;">Belum ada kategori</p>
            @endforelse
        </div>
    </div>

    <!-- Footer -->
    <div class="border-t py-8 px-6 text-center" style="border-color: #2a2a2a;">
        <div class="flex items-center justify-center gap-2 mb-2">
            <img src="{{ asset('images/logo.png') }}" alt="Logo" class="w-6 h-6 rounded-full" />
            <span class="font-semibold text-sm" style="color: #c9a84c;">Kopi Tiang Alam</span>
        </div>
        <p class="text-xs" style="color: #8b7a3c;">Menikmati kopi terbaik Nusantara</p>
    </div>
</div>
