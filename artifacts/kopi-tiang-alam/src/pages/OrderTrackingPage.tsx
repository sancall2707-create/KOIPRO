...
import { Home, Coffee, CheckCircle2, Clock, ChefHat, BellRing, Package, Wallet } from "lucide-react";
      ...
      <div className="px-4 py-5 space-y-4">
        ...
        {(order.status === "pending" || order.status === "processing") && (
          <div
            ...
            style={{ borderColor: "hsl(35,80%,70%)", background: "hsl(45,100%,96%)" }}
          >
            <div
              className="w-10 h-10 rounded-full flex items-center justify-center shrink-0"
              style={{ background: "hsl(35,90%,50%)" }}
            >
              <Wallet className="w-5 h-5" style={{ color: "white" }} />
            </div>
            <div>
              <p className="font-semibold text-sm" style={{ color: "hsl(24,10%,10%)" }}>
                Pembayaran di Kasir
              </p>
              <p className="text-xs mt-1" style={{ color: "hsl(24,10%,40%)" }}>
                Silakan lakukan pembayaran di kasir dengan menyebutkan nomor pesanan{" "}
                <span className="font-bold" style={{ color: "hsl(24,35%,25%)" }}>
                  {order.orderNumber}
                </span>. Kami menerima pembayaran tunai dan QRIS.
              </p>
            </div>
          </div>
        )}
